...
class UserService:
    ...
    def next_pk(self):
        ...
        sql = "select max(id) from sos_user1"
        cursor.execute(sql)
    ...
    def add(self, data):
        ...
        cursor = connection.cursor()
        sql = "insert into sos_user1 values(%s, %s, %s, %s, %s, %s, %s)"
    ...
    def update(self, data):
        ...
        cursor = connection.cursor()
        sql = "update sos_user1 set first_name = %s, last_name = %s,login_id = %s, password = %s, dob = %s, address = %s where id = %s"
    ...
    def delete(self, id):
        cursor = connection.cursor()
        sql = "delete from sos_user1 where id = %s"
        data = (id,)
        cursor.execute(sql, data)
        connection.commit()
        connection.close()
    ...
    def get(self, id):
        cursor = connection.cursor()
        sql = "select * from sos_user1 where id = %s"
        data = (id,)
        cursor.execute(sql, data)
        result = cursor.fetchall()
    ...
    def find_by_login(self, login_id):
        cursor = connection.cursor()
        sql = "select * from sos_user1 where login_id = %s"
        data = (login_id,)
        cursor.execute(sql, data)
        result = cursor.fetchall()
        column_name = ("id", "first_name", "last_name", "login_id", "password", "dob", "address")
    ...
    def authenticate(self, data):
        ...
        login_id=data['login_id']
        password= data['password']
        sql = "select * from sos_user1 where login_id = %s and password = %s"
    ...
    def search(self, params):
        ...
        cursor = connection.cursor()
        sql = "select * from sos_user1 where 1=1"
        if first_name != '':
